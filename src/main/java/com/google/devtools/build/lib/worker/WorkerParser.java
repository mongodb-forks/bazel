...
public class WorkerParser {
  ...
  static WorkerKey createWorkerKey(
    ...
    if ((dynamic || forceSandboxedWorkers)
    ...
    if (multiplex) {
      sandboxed =
          Spawns.supportsMultiplexSandboxing(spawn)
              && (options.multiplexSandboxing || dynamic || forceSandboxedWorkers);
    } else {
      sandboxed = options.workerSandboxing || dynamic || forceSandboxedWorkers;
    }
